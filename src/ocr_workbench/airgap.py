...
'''
    ...
    native=subprocess.run([str(a.bundle/'tools/offline_guard.exe'),'--probe'],capture_output=True,text=True,timeout=15)
    results.append({'runtime':'native_winsock','exit_code':native.returncode,'stdout':native.stdout,'stderr':native.stderr})
    ...
    (a.output/'os-isolation-result.json').write_text(json.dumps({'passed':res.returncode==0,
       ...
       'loopback':'allowed and tested','acceptance_exit_code':res.returncode},indent=2),encoding='utf-8')
