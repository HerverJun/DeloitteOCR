...
def paddle_engine(engine, models, image):
    ...
    if engine == 'ppocr':
        ...
        pipe = PaddleOCRVL(**common, pipeline_version='v1.6',
            layout_detection_model_dir=str(models / 'PP-DocLayoutV3'),
            vl_rec_model_dir=str(models / 'PaddleOCR-VL-1.6'),
            use_layout_detection=True, vl_rec_backend='native', use_queues=False)
    ...
    for page in raw:
        ...
        if engine == 'ppocr':
            for text, score, polygon in zip(data['rec_texts'], data['rec_scores'], data['rec_polys']):
                blocks.append({'text': text, 'confidence': float(score), 'polygon': polygon,
                               'kind': 'text'})
        else:
            for block in data['parsing_res_list']:
                box = block.get('block_bbox')
                polygon = [[box[0], box[1]], [box[2], box[1]], [box[2], box[3]], [box[0], box[3]]] if box else None
                blocks.append({'text': block['block_content'], 'confidence': None,
                               'polygon': polygon, 'kind': block['block_label']})
...
def main():
    ...
    p.add_argument('--engine', choices=['ppocr', 'paddlevl', 'glm', 'hunyuan'], required=True)
    ...
    try:
        ...
        engine_info = json.loads((args.bundle/'config/engines.json').read_text(encoding='utf-8'))[args.engine]
        revisions = {}
        ...
        (args.output/'raw.json').write_text(json.dumps(raw,ensure_ascii=False,indent=2,default=json_safe),encoding='utf-8')
        tables = parse_tables(text)
        result = {'schema_version': 1, 'status': 'success', 'engine': args.engine,
                  'engine_info': engine_info, 'model_revisions': revisions,
                  'image': {'source': str(args.image.resolve()), 'sha256': original_hash,
                            'version': hashlib.file_digest(prepared.open('rb'), 'sha256').hexdigest(),
                            'width': width, 'height': height, 'coordinates': 'prepared_image_pixels'},
                  'elapsed_seconds': time.perf_counter() - started, 'load_seconds': loaded,
                  'text': text, 'blocks': blocks, 'tables': tables, 'raw': raw}
        ...
        pending.write_text(json.dumps(result, ensure_ascii=False, indent=2, default=json_safe), encoding='utf-8')
        pending.replace(args.output / 'result.json')
        faulthandler.cancel_dump_traceback_later()
    except Exception as error:
        (args.output / 'error.json').write_text(json.dumps({'status': 'failed', 'engine': args.engine,
